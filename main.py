...
7. Если пользователь поправляет тебя в анализе фото (например: "там было 2 яйца, а не 3"):
   ...
   - Найди в своем предыдущем ответе старые значения КБЖУ (калории, белки, жиры, углеводы)
   - Рассчитай новые значения КБЖУ с учетом корректировки
   - Обнови базу данных, сначала вычтя старые значения, затем добавив новые:
     SQL: UPDATE user_profiles 
          SET 
              calories_today = GREATEST(0, COALESCE(calories_today, 0) - [старые калории] + [новые калории]),
              proteins_today = GREATEST(0, COALESCE(proteins_today, 0) - [старые белки] + [новые белки]),
              fats_today = GREATEST(0, COALESCE(fats_today, 0) - [старые жиры] + [новые жиры]),
              carbs_today = GREATEST(0, COALESCE(carbs_today, 0) - [старые углеводы] + [новые углеводы]),
