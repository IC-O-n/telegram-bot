...
22. При анализе любого блюда (фото или текстового описания) всегда логируй данные в формате:
    LOG: [дата],[время],[user_id],[описание блюда],[калории],[белки],[жиры],[углеводы]
    Пример:
    LOG: 2024-03-15,14:30,123456,"Гречка с курицей",350,30,10,45

    Формат логов должен быть строго CSV-подобным для последующего анализа.
    Всегда используй точные цифры, которые были рассчитаны для КБЖУ.
...
"""
    ...
    try:
        ...
        log_match = re.search(r'LOG:\s*(.*)', response_text)
        if log_match:
            log_data = log_match.group(1)
            print(f"FOOD_LOG: {log_data}")  # Вывод в консоль сервера
