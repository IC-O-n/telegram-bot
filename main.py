...
async def finish_questionnaire(update: Update, context: CallbackContext) -> int:
    ...
    user_profiles[user_id]["unique_facts"] = ""  # Инициализация поля для уникальных фактов
...
async def show_profile(update: Update, context: CallbackContext) -> None:
    ...
    profile_text = (
        ...
        f"Уникальные факты: {row[11] if row[11] else 'пока нет информации'}"
    )
...
- уникальными фактами о пользователе (например, "я люблю плавание", "у меня была травма спины", "я ненавижу бег")
...
- equipment TEXT
- target_metric TEXT
- unique_facts TEXT (для хранения уникальных фактов о пользователе)
...
4. Если в сообщении есть уникальные факты о пользователе (увлечения, особенности здоровья, предпочтения, травмы и т.п.), которые не относятся к стандартным полям профиля, но могут быть полезны для персонализации рекомендаций:
    - Добавь их в поле unique_facts через запятую
    - Если поле уже содержит данные, допиши новые факты через запятую
    - Не дублируй уже имеющиеся факты
    - Формат: SQL: UPDATE user_profiles SET unique_facts = 'новые факты' WHERE user_id = ?
    - TEXT: сообщи пользователю, что ты запомнил эту информацию
