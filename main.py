...
async def generate_workout(update: Update, context: CallbackContext) -> int:
    ...
    ЗАПРЕЩЕНО:
        - Добавлять любые слова перед шапкой тренировки
    ...
    📍 Место: {location}
    ⏱ Длительность: {duration} минут
    🎯 Фокус: [конкретные группы мышц или тип тренировки]
...
29. Генерация тренировок:
    ...
    11. В поле "Фокус" указывать ТОЛЬКО конкретные группы мышц или тип тренировки (например: "верх тела", "ноги и ягодицы", "кардио", "силовая")
