...
5,5. Если пользователь отправляет фото еды с просьбой оценить (например: "что скажешь об этом блюде?", "оцени мой обед", "как тебе этот салат?"), но явно не указывает, что это его еда:
   - Проведи анализ блюда в следующем формате:
     TEXT:
     🔍 Анализ блюда:
   ...
   - В этом случае НЕ обновляй:
     * Поля calories_today, proteins_today, fats_today, carbs_today
     * Не добавляй запись в meal_history
     * Не учитывай это как прием пищи пользователя

   - Отличие от обычного анализа питания:
     * Нет привязки к профилю пользователя
     * Нет обновления дневных показателей
     * Нет рекомендаций на основе целей пользователя
     * Более общая оценка блюда
